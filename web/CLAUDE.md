...
| `addInputRow(proc, focusFirst)` | 입력행 추가 (Enter → 다음 행, 방향키/Tab → 칸 이동) |
...
| `applyCFD(tableId, colIdx, dropdown)` | 컬럼 필터 적용 |
| `applyAllColFilters(tableId)` | 테이블 전체 컬럼 필터 일괄 적용 |
| `updateCFIndicators(tableId)` | 필터 활성 컬럼 헤더 표시 업데이트 |
...
|------|-----|
| 📈 대시보드 | — |
| 📋 공정진행표 | 전체 현황 / 분류 / 면표시 / 문서스캔 / 도면스캔 / 보정 / 색인 / 재편철 / 공개구분 |
| 📅 일별 총괄표 | — (독립 메뉴) |
| 👥 작업자별 현황 | — |
| 🔍 품질검사 | — |
| ⚙️ 설정 | — |
...
## 입력폼 키보드 동작
- **Enter** → 어느 칸에서든 새 행 레이블 칸으로 이동
- **Tab / Shift+Tab** → 같은 행 다음/이전 칸 이동 (브라우저 기본)
- **→ 방향키** (커서 끝) → 다음 칸 이동
- **← 방향키** (커서 처음) → 이전 칸 이동
...
- **정렬 필터**: 이름순 / 작업량 많은순 / 작업량 적은순
- **전체 선택 시**: 공정×작업자 피벗 테이블. 분류는 권호수·건 **두 컬럼**으로 분리. 차트 없음
- **개별 공정 선택 시**: 작업자별 권호수·수량 집계 + 바 차트 + 일별 추이 라인 차트
- 단위: 분류·색인·재편철·공개구분 = 건, 면표시·스캔·보정 = 면
- `window._wsRows = {date, worker, proc, kwon, qty}` 배열로 데이터 캐시
...
- 헤더 클릭 → 고유값 체크박스 드롭다운 → 적용/초기화
- 드롭다운 내 검색창으로 값 빠르게 탐색
- 활성 필터 컬럼 헤더 색상 강조 (`cf-active`)
- `window._colFilters = { 'tableId:colIdx': Set }` 로 상태 관리
- 여러 컬럼 동시 필터링 가능 (AND 조건)

---

## UI 패턴
- **Enter 자동 행 추가**: 입력폼 어느 칸에서든 Enter → 다음 행 레이블 칸으로 이동
...
| `.col-filter-dropdown` | 컬럼 드롭다운 필터 컨테이너 |
